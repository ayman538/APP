...
public class RedHybrid_ViewAllowance_Android extends RedHybrid_ViewAllowance_Abstract {
    ...
    public void assertOnTotalGBForPromotedPlan() {
        ...
        String actualToltalAmount = dashboardV4ResponseModel.tiles.get(0).content.get(0).title;
        int sizeOFUsages = usagesResponseModel.cards.get(0).sections.size();

        for (int i = 0; i < sizeOFUsages; i++) {
            if (usagesResponseModel.cards.get(1).sections.get(0).header.contains("Data - Promo extra")) {
        ...
        Assert.assertTrue(actualToltalAmount.contains(expectedTotalGB));
