...
public class RedHybrid_ConfirmYourPurchase_iOS extends RedHybrid_ConfirmYourPurchase {
    ...
    By passCode = By.id("passcode");
    By authSubmitBtn = By.xpath("//input[@type='submit'])");
    ...
    public void confirmPurchaseScreenIsDisplayed() {
        waitForVisibility(confirmPlanPurchaseProductName);
...
//      submitCvvAndHolderName(URL, "234");
    ...
    public void submitCvvAndHolderName(String URL, String cvv) {
        ...
        driver.get(URL);
        WebUtils.waitForVisible(driver, driver.findElement(By.id("confirmPaymentDetailsHeader")));
        driver.findElement(confirmCardHolderName).sendKeys("xxx");
        driver.findElement(confirmCsc).sendKeys(cvv);
        driver.findElement(submitBtn).click();

        try {
            WebUtils.waitForVisible(driver, driver.findElement(passCode));
        ...
        try {
            driver.findElement(authSubmitBtn);
        } catch (Exception e) {
            System.out.println("no auth needed");

        }
        WebUtils.implicitWait(driver, 5);
        driver.quit();
    }
}
