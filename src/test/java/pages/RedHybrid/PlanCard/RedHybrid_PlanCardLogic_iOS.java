...
public class RedHybrid_PlanCardLogic_iOS extends RedHybrid_PlanCard {
    ...
    public RedHybrid_PlanCardLogic_iOS() {
        super();
    }
    ...
    public void ValidateIncludesDisplayingAfterClickingShowmore() {
        ...
        if (Plan.cards.size() > 2) {
            ...
            if (Plan.cards.get(0).sections.get(1).list.size() <= 3) { //in cas user has 3 includes or less
                ...
                //to get the iOS identifier for the Includes items
                Assert.assertNotEquals(IncludesContainer.findElements(By.className("XCUIElementTypeTable")).size(), 3);
            }
        }
    ...
    public CreateOrderResponseModel ToGetCreateOrderResponse() throws JsonProcessingException {
        ...
        System.out.println(hybridBundelsResponseModel.status.analytics.guid + "  GUID");
    ...
    public void checkContentOfTealCard() {
        ...
        Assert.assertEquals(planV3ResponseModel.cards.get(0).iconId, "10161");
    ...
    public void iAmARedhybridUserWhoTurnsOffTheAutoRenewToggle() {
        scrollDownTo(RedHybrid_AutoRenewToggle);
        waitForVisibility(RedHybrid_AutoRenewToggle);
        doubleClick(RedHybrid_AutoRenewToggle);
    }
