...
public class Change_Next_Plan_Change_Next_Plan_overlay_Logic_Android extends Change_Next_Plan_Change_Next_Plan_overlay_Abstract {

    HybridBundelsResponseModel hybridBundelsResponseModel;
    Map<String, String> queryParameters;

    public Change_Next_Plan_Change_Next_Plan_overlay_Logic_Android() {
        super();
    }
    ...
    public void changePlanCTAClick() {
       ...
       scrollDownTo(planTabChangePlanCTA);
        waitForVisibility(planTabChangePlanCTA);
        planTabChangePlanCTA.click();
    }

//    @Override
//    public void clickChangeNextPlanCTA() {
//        androidScrollToElement(changeNextPlanQuickLinks, "down");
...
//    }
    ...
    public void screenUIValidation() throws InterruptedException {
        ...
        int i;
        int planItems = changePlanChoosePlanRecyclerView.findElements(By.id("com.myvodafoneapp:id/planItem")).size();
        for (i = 0; i < planItems; i++) {
            Assert.assertFalse(changePlanChoosePlanRecyclerView.findElements(By.id("com.myvodafoneapp:id/planItem")).get(i).isSelected());
        }
    ...
    public void checkTheUIOfCellWithPromotedPlan(HybridBundelsResponseModel hybridBundelsResponseModel) {
        ...
        for(int plans=0; plans < hybridBundelsResponseModel.items.size() ; plans++){
            ...
            Assert.assertTrue(changePlanChoosePlanChargesNotes.getText().contains("Promo terms apply"));
        }
    ...
    public void clickOnPromosTermsApplyHyperLink() {
        if (changePlanChoosePlanChargesNotes.getText().contains("Promo terms apply")){
            try{
                ...
                System.out.println("No Promo terms Web screen appears");
            }
        }
    }
