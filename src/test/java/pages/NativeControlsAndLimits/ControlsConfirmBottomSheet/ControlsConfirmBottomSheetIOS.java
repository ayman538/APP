...
public class ControlsConfirmBottomSheetIOS extends ControlsConfirmBottomSheetAbstract {

    public static String controlName;

    NativeControlsResponseModelAnalyticsVersion nativeControlsResponseModel = NativeControlsUtils.getNativeControlsResponseModelAnalyticsVersion();
    ...
    public void navigateToZeroLvlControlConfirmation() throws Exception {
        ...
        for(int i=0;i<categories.size();i++){
            ...
            if (nativeControlsSubGroupProductsResponseModel.confirmControl != null){
                ...
                controlName = nativeControlsSubGroupProductsResponseModel.confirmControl.name;
                category.click();
                break;
            }
        }
            ...
            if(nativeControlsSubGroupProductsResponseModel.itemsGroup != null){
                ...
                for(j=0;j<firstSubcategories;j++){
                    if (nativeControlsSubGroupProductsResponseModel.itemsGroup.items.get(j).confirmControl != null){
                        ...
                        controlName = nativeControlsSubGroupProductsResponseModel.itemsGroup.items.get(j).confirmControl.name;
                        WebElement subcategory = driver.findElement(By.xpath("//*[@label='"+subcategoryText+"']"));
    ...
    public void validateConfirmTurnOffControlTrayIsOpened(String buttonName){
        for (int i = 0; i < nativeControlsResponseModelPhase1.activeProducts.activeProductsDetails.size(); i++) {
            if (nativeControlsResponseModelPhase1.activeProducts.activeProductsDetails.get(i).name.equalsIgnoreCase(NativeControlsAndLimitsLogiciOS.controlName)) {
                assertDisplayed(confirmControlsScreenTitle,3);
